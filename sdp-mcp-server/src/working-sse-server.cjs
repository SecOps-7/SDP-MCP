...
// MCP Resources — sourced from SDP-MCP_Usage.md
const SDP_RESOURCES = {
  'sdp://usage/api-rules': {
...
search_requests — Search requests using simple criteria.
  Params: search_fields (object), row_count, start_index
...
reply_to_requester — Send email reply visible in the ticket conversation thread.
  Required: request_id, reply_content
...
3. Use Bash tool for both`;
  ...
  async get_usage_guide(params) {
    ...
    const resource = SDP_RESOURCES[uri];

    if (!resource) {
      const valid = Object.keys(SDP_RESOURCES).map(u => u.replace('sdp://usage/', '')).join(', ');
    ...
    return {
      content: [
        {
          type: 'text',
          text: `Usage guide section: ${resource.name}\n\n${resource.text}`
        },
        {
          type: 'resource',
          resource: {
...
function handleJsonRpcMessage(message, sseConnection) {
  ...
  try {
    ...
    switch (method) {
      ...
      case 'resources/list':
        result = {
          resources: Object.values(SDP_RESOURCES).map(r => ({
            uri: r.uri,
            name: r.name,
      ...
      case 'resources/read': {
        ...
        const resource = SDP_RESOURCES[resourceUri];
        if (!resource) {
          return {
            jsonrpc: '2.0',
            error: { code: -32602, message: `Unknown resource URI: ${resourceUri}` },
            id
          };
        }
        result = {
